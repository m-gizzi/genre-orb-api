...
RSpec.describe SyncSession do
  describe "#progress" do
    ...
    it "returns zero progress when no playlists" do
      ...
      expect(progress).to eq({ total: 0, completed: 0, percent: 0 })
    end
    ...
    it "calculates progress correctly" do
      ...
      progress = session.progress
      expect(progress).to eq({ total: 3, completed: 2, percent: 66 })
    end
    ...
    it "returns 100 percent when all complete" do
      ...
      progress = session.progress
      expect(progress).to eq({ total: 1, completed: 1, percent: 100 })
    end
  end
